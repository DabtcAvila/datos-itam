...
// y en cada /consar/<slug>. Pills horizontales scroll-x para no chocar
// visualmente con el nav nivel-1 (.dataset-tabs).
...
export interface ConsarSubnavItem {
  ...
  datasetN: string;    // numeración CONSAR (#01..#11) — etiqueta secundaria
}

export const CONSAR_SUBNAV: ConsarSubnavItem[] = [
...
export function buildConsarSubnav(activeSlug: string): string {
  const items = CONSAR_SUBNAV.map(it => {
    const cls = it.slug === activeSlug ? 'consar-subnav-pill active' : 'consar-subnav-pill';
    const aria = it.slug === activeSlug ? ' aria-current="page"' : '';
    return `<a href="${it.path}" class="${cls}"${aria}>
      <span class="consar-subnav-label">${it.label}</span>
      <span class="consar-subnav-meta">${it.datasetN}</span>
    </a>`;
  }).join('');
  return `<nav class="consar-subnav" aria-label="Datasets CONSAR">${items}</nav>`;
}
